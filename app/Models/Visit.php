...
class Visit extends BaseModel
{
    ...
    public function scopeGetVisitsEloquentObjWithOutSelect($query, $request)
    {
        return $query->join('company_units', 'company_units.id', '=', 'visits.company_unit_id')
            ...
            ->join('users as assaigned', 'assaigned.id', '=', 'visits.assign_to')
            ...
            ->when($request->date_for, function ($q) use ($request) {
                ...
                if (!empty($segments[0]) && !empty($segments[1])) {
                    ...
                    $q->whereBetween('visits.date_for', [$min, $max]);
                }
            })
            ...
            ->when(
                ...
                function ($q) use ($request) {
                    $q->where(function ($q1) use ($request) {
                        $q1->where('visits.assign_to', $request->user()->id)
    ...
    {
        return self::getVisitsEloquentObjWithOutSelect($request)
            ->select(
                'visits.*',
