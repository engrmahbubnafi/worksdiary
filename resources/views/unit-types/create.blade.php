<x-app-layout>
    ...
    <x-slot name="subheader">
        <x-subheader-comp>
            Create New Unit Type
            @slot('actions')
                {!! Html::decode(link_to_route('unit-types.index', 'Unit Types', null, ['class' => 'btn btn-sm btn-light'])) !!}
            @endslot
        </x-subheader-comp>
    </x-slot>
    ...
    <div id="kt_content_container" class="container-xxl">
        <div class="card">
            <div class="card-body p-lg-15">            
                ...
                {{ Form::model(request()->old(), ['route' => 'unit-types.store', 'class' => 'form', 'method' => 'POST']) }}
                                ...
                                <div class="js-actions">
                                    <label class="form-check form-check-custom form-check-solid me-10">
                                        <input id="checkoruncheck" class="form-check-input h-20px w-20px department"
                                            type="checkbox" checked="checked" name="department_ids[]"
                                            value="{{ $department->id }}"
                ...
                @else
                    <div class="mb-7 text-center">
                        <h1 class="mb-3">Department not available! please add department</h1>
                    </div>
                @endif               
